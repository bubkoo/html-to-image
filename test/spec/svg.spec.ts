...
describe('work with svg element', () => {
  ...
  xit('should render svg `<rect>` with width and heigth', (done) => {
    bootstrap('svg-rect/node.html', 'svg-rect/style.css', 'svg-rect/image')
      .then((node) => {
        // Use a higher threshold for SVG elements as they can vary between browsers
        return toPng(node).then((dataUrl) => {
          return Promise.resolve(dataUrl)
            .then(drawDataUrl)
            .then((imgData) => compareToRefImage(imgData, 0.2)) // Increased threshold
        })
      })
      .then(done)
      .catch(done)
